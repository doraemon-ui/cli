...
const copy = (paths) => () => gulp.src(paths.entry).pipe(convertJson()).pipe(gulp.dest(paths.outputDir))

const scripts = (paths) => () => gulp.src(paths.entry).pipe(tsProject()).js.pipe(banner(util.banner())).pipe(gulp.dest(paths.outputDir))
...
const watch = () => {
  ...
  gulp.watch(
    config.cssPlugin.entry,
    styles({ entry: config.cssPlugin.entry, outputDir: config.outputDir, pxTransform: config.cssPlugin.pxTransform }),
  )
}
...
function logEvents(gulpInst, onListening) {
  ...
  gulpInst.on('stop', async function (evt) {
    ...
    /* istanbul ignore next */
    const level = evt.branch ? 'debug' : 'info'
    console[level]('Finished', "'" + ansi.cyan(evt.name) + "'", 'after', ansi.magenta(time))
  })
  ...
  gulpInst.on('error', async function (evt) {
    ...
    console[level]("'" + ansi.cyan(evt.name) + "'", ansi.red('errored after'), ansi.magenta(time))
